...
trait dependencies {
  ...
  def dep(lib: String): ModuleID = libs(lib).toModuleId

  def depJava(lib: String): ModuleID = libs(lib).toModuleId.cross(CrossVersion.Disabled)

  def depJS(lib: String): ModuleID = libs(lib).toJsModuleId
